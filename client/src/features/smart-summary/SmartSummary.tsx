...
export const SmartSummary: React.FC<Props> = ({ summary, loading }) => {
  ...
  return (
    <div className="bg-violet-50 border border-violet-100 rounded-lg shadow-sm overflow-hidden">
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="w-full flex items-center justify-between gap-2 p-4 hover:bg-violet-100/50 transition-colors"
      >
        <div className="flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-violet-600" />
          <h3 className="font-semibold text-violet-900">HALO Smart Summary</h3>
        </div>
        {collapsed ? (
      ...
      {!collapsed && (
        <div className="px-4 pb-4">
          {loading ? (
            <div className="flex items-center gap-2 text-violet-700 py-2">
